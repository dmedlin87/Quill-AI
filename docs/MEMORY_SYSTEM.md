...
- **Bedside-note evolution (Phase 2, thin slice)**
  ...
  - Evolution is currently driven by three main flows:
    ...
    - **Goal lifecycle:** When goals are created, completed, or abandoned via `services/memory/index.ts`, `syncGoalLifecycleUpdate` calculates active vs. total goals and evolves the bedside note with a concise update (e.g., "Goal completed: [Title] — consider next steps") and `changeReason: 'goal_lifecycle'`.
