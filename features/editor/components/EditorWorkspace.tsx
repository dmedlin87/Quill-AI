...
export const EditorWorkspace: React.FC = () => {
  ...
  const handleFixWithAgent = async (
    ...
    const context = quote ? `"${quote}"` : 'the selected text';
    const prompt = `Fix the following issue: ${issue}. Context: ${context}. Suggestion: ${suggestion}`;

    try {
      await engineActions.handleAgentAction('fix_issue', {
        issue,
        suggestion,
        quote,
        prompt
      ...
      console.error('Failed to fix with agent:', error);
    }
  };
