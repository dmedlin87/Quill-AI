...
export const AnalysisPanel: React.FC<AnalysisPanelProps> = ({ analysis, isLoading, currentText, onNavigate, onFixRequest, warning, onAnalyzeSelection, hasSelection, contradictions = [], derivedLore }) => {
  ...
  return (
    <div className="h-full overflow-y-auto p-5 space-y-6 animate-fade-in">
      {/* API Key Warning */}
      {!isApiConfigured() && (
        <div className="flex items-start gap-3 p-3 rounded-lg border border-red-200 bg-red-50 text-red-900 animate-slide-up">
          <div className="mt-0.5 text-red-600 font-bold">!</div>
          <div className="flex-1 space-y-1">
            <p className="text-[var(--text-sm)] font-semibold text-red-800">API Key Missing</p>
            <p className="text-[var(--text-xs)] leading-relaxed text-red-700">
              Gemini API key is not configured. Analysis features are disabled.
              Please add <code>GEMINI_API_KEY</code> to your environment or settings.
            </p>
          </div>
        </div>
      )}
                  ...
                  {onAnalyzeSelection && (
                    ...
                    >
                      <button
                        ...
                        className={`px-3 py-1.5 text-[var(--text-xs)] font-medium rounded-md transition-colors ${
                          hasSelection
                            ? 'bg-[var(--warning-200)] text-[var(--warning-900)] hover:bg-[var(--warning-300)]'
                            : 'bg-[var(--warning-100)] text-[var(--warning-400)] cursor-not-allowed'
                        }`}
                      >
                        Analyze Selection
                      </button>
